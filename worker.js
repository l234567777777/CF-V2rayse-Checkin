...
let user = "这里填邮箱";
let pass = "这里填密码";
let siteType = "v2rayse"; // 站点类型: sspanel | v2rayse
let BotToken = '';
let ChatID = '';
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname.startsWith("/api/")) {
      return handleApiRequest(request, url.pathname, ctx);
    }
...
async function initializeVariables(env) {
  ...
  siteType = env.TYPE || siteType;
  if (!domain.includes("//")) domain = `https://${domain}`;
  BotToken = env.TGTOKEN || env.TG_TOKEN || BotToken;
  ChatID = env.TGID || env.TG_ID || ChatID;
}
...
// --- API Router ---
async function handleApiRequest(request, pathname, ctx) {
  ...
  try {
    switch (pathname) {
      ...
      case "/api/checkin":
        ...
        // 手动签到也触发 TG 推送
        if (ChatID) {
          ctx.waitUntil(sendMessage(res.result));
        }
...
async function performCheckinWithLogs() {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      log(`[第 ${attempt}/${maxRetries} 次尝试] 站点类型: ${siteType}`);
      ...
      if (siteType === 'v2rayse') {
        return await performV2rayseCheckin(log, capturedLogs);
      } else {
        return await performSspanelCheckin(log, capturedLogs);
      }
...
// --- SSPanel Checkin (Original) ---
async function performSspanelCheckin(log, capturedLogs) {
  const UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/129.0.0.0 Safari/537.36';

  log("[SSPanel] 访问站点获取初始会话...");
  ...
  log(`[SSPanel] 请求登录接口: ${domain}/auth/login`);
  ...
  for (let sc = 1; sc <= maxSessionChecks; sc++) {
    ...
    if (verifyStatus === 200 || verifyStatus === 304) {
      ...
      if (sc < maxSessionChecks) {
        log(`会话尚未就绪 (HTTP ${verifyStatus})，等待重试... (${sc}/${maxSessionChecks})`);
      } else {
        log(`会话始终未就绪 (HTTP ${verifyStatus})，已尝试 ${maxSessionChecks} 次`);
      }
    } else {
  ...
  try {
    ...
    throw new Error(`解析签到响应失败 (HTTP ${checkinResponse.status}): ${responseText.substring(0, 200)}...`);
  }
...
// --- V2rayse Checkin (Nuxt-based API) ---
async function performV2rayseCheckin(log, capturedLogs) {
  const UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/129.0.0.0 Safari/537.36';

  // Step 1: Login via API
  log(`[v2rayse] 请求登录接口: ${domain}/api/auth/login`);
  const loginResponse = await fetch(`${domain}/api/auth/login`, {
    ...
    body: JSON.stringify({ email: user, password: pass, remember: true }),
  });

  if (!loginResponse.ok) {
    const errText = await loginResponse.text();
    throw new Error(`登录失败 (HTTP ${loginResponse.status}): ${errText.substring(0, 200)}`);
  }
  ...
  try {
    ...
    throw new Error(`登录响应解析失败: ${text.substring(0, 200)}`);
  }

  // v2rayse returns user object directly on success (contains user.id)
  if (!loginJson.user && !loginJson.success) {
    throw new Error(`登录校验失败: ${loginJson.message || loginJson.msg || JSON.stringify(loginJson)}`);
  }
  log("✓ 登录验证通过");

  // Extract nuxt-session cookie
  const loginCookies = extractCookies(loginResponse);
  if (loginCookies.length === 0) {
    throw new Error("登录后未获取到 Cookie (nuxt-session)");
  }
  log(`登录后 Cookie (${loginCookies.length}): ${cookieNameList(loginCookies)}`);

  // Step 2: Checkin
  await new Promise(resolve => setTimeout(resolve, 1000));
  log("正在发送签到请求...");
  log(`签到使用 Cookie: ${cookieNameList(loginCookies)}`);

  const checkinResponse = await fetch(`${domain}/api/account/points/check-in`, {
    method: 'POST',
    headers: {
  ...
  if (checkinResponse.status >= 300 && checkinResponse.status < 400) {
    const location = checkinResponse.headers.get('location') || '未知';
    throw new Error(`签到请求被重定向 (HTTP ${checkinResponse.status}) -> ${location}，Session 失效`);
  }
  ...
  try {
    ...
    log(`JSON 解析失败，原始响应: ${responseText.substring(0, 300)}`);
    // 不是 JSON 且状态码错误，才抛异常
    if (!checkinResponse.ok && checkinResponse.status !== 409) {
      throw new Error(`签到请求失败 (HTTP ${checkinResponse.status}): ${responseText.substring(0, 200)}`);
    }
    // 状态码 409 或 ok，即使不是 JSON 也算已签到/成功
    const msg409 = checkinResponse.status === 409 ? '✅ 今天已经签过了，明天再来吧~' : `签到完成 (HTTP ${checkinResponse.status})`;
    const finalMsg = `[签到回报] ${msg409}`;
    log(finalMsg);
    return { success: true, result: finalMsg, logs: capturedLogs };
  }

  log(`签到响应解析后对象: ${JSON.stringify(checkinResult)}`);

  // --- 成功判定（从宽）---
  // 情况1: HTTP 409 → 已签到
  // 情况2: 响应体有 awardedPoints → 获得积分
  // 情况3: 响应体 message 包含 "成功" 或 "积分" 字样 → 业务成功
  // 情况4: 响应体 message 包含 "已经" 或 "已签" 或 "already" → 已签到
  // 情况5: HTTP 2xx 且能正常解析 JSON → 宽松算成功（避免误判为失败）
  const responseMsg = (checkinResult.message || checkinResult.msg || '').toLowerCase();
  const hasSuccessKeyword = responseMsg.includes('成功') || responseMsg.includes('积分');
  const hasAlreadyKeyword = responseMsg.includes('已经') || responseMsg.includes('已签') || responseMsg.includes('already') || responseMsg.includes('done');

  let isSuccess = false;
  let msg = '';

  // 检查 summary 结构（v2rayse 新版返回格式）
  const summary = checkinResult.summary || null;
  const pointsFromSummary = summary && summary.points !== undefined ? summary.points : null;

  if (checkinResponse.status === 409) {
    isSuccess = true;
    msg = '✅ 今天已经签过了，明天再来吧~';
  } else if (checkinResult.awardedPoints !== undefined) {
    isSuccess = true;
    msg = `获得 ${checkinResult.awardedPoints} 积分`;
  } else if (pointsFromSummary !== null) {
    isSuccess = true;
    const streak = summary.checkInStreak !== undefined ? `，连续签到 ${summary.checkInStreak} 天` : '';
    msg = `✅ 签到成功！当前积分: ${pointsFromSummary}${streak}`;
  } else if (hasAlreadyKeyword) {
    isSuccess = true;
    msg = checkinResult.message || checkinResult.msg || '今天已经签过了';
  } else if (hasSuccessKeyword) {
    isSuccess = true;
    msg = checkinResult.message || checkinResult.msg || '签到成功';
  } else if (checkinResponse.ok) {
    // HTTP 2xx 但没有明确的成功标志 → 宽松处理，算成功但标注不确定
    isSuccess = true;
    msg = checkinResult.message || checkinResult.msg || `签到完成 (HTTP ${checkinResponse.status}, 响应: ${responseText.substring(0, 100)})`;
  } else {
    // 确实失败了
    isSuccess = false;
    msg = checkinResult.message || checkinResult.msg || `签到失败 (HTTP ${checkinResponse.status})`;
  }

  const finalMsg = `[签到回报] ${msg}`;
  log(finalMsg);
  log(`最终判定: success=${isSuccess}, httpStatus=${checkinResponse.status}, awardedPoints=${checkinResult.awardedPoints}, message="${checkinResult.message || checkinResult.msg || '(空)'}"`);

  return { success: isSuccess, result: finalMsg, logs: capturedLogs };
}
...
async function sendMessage(msg = "") {
  ...
  } else if (ChatID !== "" && ChatID !== undefined && ChatID !== null) {
